...
from dmrgat.graph_builder import GraphDataset
from dmrgat.utils import get_device, set_seed

from model import IndustryLSTM
...
class SequenceTensorDataset(Dataset):
    def __init__(self, graph_dataset: GraphDataset, sample_indices: list[int], seq_len: int) -> None:
        self.graphs = graph_dataset.graphs
    ...
    def __getitem__(self, index: int) -> tuple[torch.Tensor, torch.Tensor]:
        end_idx = self.sample_indices[index]
        start_idx = end_idx - self.seq_len + 1
        x_seq = torch.stack([self.graphs[i].x.float() for i in range(start_idx, end_idx + 1)], dim=0)
        y = self.graphs[end_idx].y.float()
        return x_seq, y


def _make_sequence_splits(graph_dataset: GraphDataset, config: ProjectConfig, seq_len: int) -> dict[str, list[int]]:
    total = len(graph_dataset.graphs)
    if total < seq_len + 2:
        raise ValueError("Not enough graph snapshots to build LSTM sequence samples.")

    sample_indices = list(range(seq_len - 1, total))
    sample_total = len(sample_indices)
    train_end = max(int(sample_total * config.train_ratio), 1)
    val_end = max(int(sample_total * (config.train_ratio + config.val_ratio)), train_end + 1)
    val_end = min(val_end, sample_total - 1)
    return {
        "train": sample_indices[:train_end],
        "val": sample_indices[train_end:val_end],
        "test": sample_indices[val_end:],
    }
...
def _make_loader(
    ...
    dataset = SequenceTensorDataset(graph_dataset, sample_indices, seq_len)
...
def _evaluate(
    ...
    daily_ic = []
    daily_rank_ic = []
    loader = _make_loader(graph_dataset, sample_indices, seq_len, batch_size, shuffle=False)
...
def fit_and_evaluate_lstm(
    ...
    num_layers: int = 1,
) -> tuple[LSTMRunResult, IndustryLSTM]:
    set_seed(config.seed)
    splits = _make_sequence_splits(graph_dataset, config, seq_len)
    ...
    train_loader = _make_loader(graph_dataset, splits["train"], seq_len, batch_size, shuffle=True)
    best_state = None
    best_val = float("-inf")
    ...
    for _ in range(config.epochs):
        ...
        val_metrics = _evaluate(model, graph_dataset, splits["val"], seq_len, device, batch_size)
    ...
    train_metrics = _evaluate(model, graph_dataset, splits["train"], seq_len, device, batch_size)
    val_metrics = _evaluate(model, graph_dataset, splits["val"], seq_len, device, batch_size)
    test_metrics = _evaluate(model, graph_dataset, splits["test"], seq_len, device, batch_size)
    ...
    result = LSTMRunResult(
        ...
        num_samples=sum(len(v) for v in splits.values()),
...
def save_metrics(metrics: LSTMRunResult, output_path: str | Path) -> None:
